...
#pragma parameter cornersmooth                                                 \
    "CRTGeom Corner Smoothness" 1000.0 80.0 2000.0 100.0
...
#pragma parameter interlace_detect                                             \
    "CRTGeom Interlacing Simulation" 1.0 0.0 1.0 1.0
...
float intersect(vec2 xy) {
  ...
  float B =
      2.0 * (R * (dot(xy, sinangle) - d * cosangle.x * cosangle.y) - d * d);
...
vec3 maxscale() {
  ...
  vec2 lo =
      vec2(fwtrans(vec2(-a.x, c.y)).x, fwtrans(vec2(c.x, -a.y)).y) / aspect;
  vec2 hi =
      vec2(fwtrans(vec2(+a.x, c.y)).x, fwtrans(vec2(c.x, +a.y)).y) / aspect;
...
void main() {
  ...
  // shader.
  sinangle =
      sin(vec2(x_tilt, y_tilt)) +
      vec2(0.001); // sin(vec2(max(abs(x_tilt), 1e-3), max(abs(y_tilt), 1e-3)));
  cosangle =
      cos(vec2(x_tilt, y_tilt)) +
      vec2(0.001); // cos(vec2(max(abs(x_tilt), 1e-3), max(abs(y_tilt), 1e-3)));
  stretch = maxscale();
...
vec3 saturation(vec3 textureColor) {
  ...
  if (lum < 0.5)
    luminanceWeighting.rgb = (luminanceWeighting.rgb * luminanceWeighting.rgb) +
...
#define pwr                                                                    \
  vec3(1.0 /                                                                   \
           ((-0.7 * (1.0 - scanline_weight) + 1.0) * (-0.5 * DOTMASK + 1.0)) - \
       1.25)
...
void main2() {
  ...
  vec2 ilvec =
      vec2(0.0, ilfac.y * interlace_detect > 1.5 ? mod(float(FrameCount), 2.0)
                                                 : 0.0);
  ...
  // pixel.
  vec4 coeffs = PI * vec4(1.0 + uv_ratio.x, uv_ratio.x, 1.0 - uv_ratio.x,
                          2.0 - uv_ratio.x);
  ...
  vec4 col = clamp(mat4(TEX2D(xy + vec2(-one.x, 0.0)), TEX2D(xy),
                   ...
                   0.0, 1.0);
  vec4 col2 =
      clamp(mat4(TEX2D(xy + vec2(-one.x, one.y)), TEX2D(xy + vec2(0.0, one.y)),
                 TEX2D(xy + one), TEX2D(xy + vec2(2.0 * one.x, one.y))) *
                coeffs,
            0.0, 1.0);
  ...
  vec3 dotMaskWeights =
      mix(vec3(1.0, 1.0 - DOTMASK, 1.0),
          vec3(1.0 - DOTMASK, 1.0, 1.0 - DOTMASK), floor(mod(mod_factor, 2.0)));
