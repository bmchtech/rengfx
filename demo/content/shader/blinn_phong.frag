...
vec3 AmbientLighting() { return (matAmbientColor * lightAmbientColor); }
...
vec3 SpecularLighting(in vec3 N, in vec3 L, in vec3 V) {
  ...
  // Calculate specular reflection only if the surface is oriented to the light
  // source
  if (dot(N, L) > 0) {
  ...
  return (matSpecularColor * lightSpecularColor * lightSpecIntensity *
          specular);
}
...
void main() {
  ...
  finalColor =
      vec4(texelColor.rgb * (ambient + diffuse + specular), texelColor.a);
}